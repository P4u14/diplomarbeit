...
class SegmentsCenterErrorRightMetric(SegmentsCenterErrorMetric):
    """
    Computes the absolute distance (in pixels) between the ground truth and predicted centroid for all segments on the right side of the back.
    This metric quantifies the spatial deviation between predicted and true segment centers on the right.
    """
    ...
    def name(self) -> str:
        ...
        Returns:
            str: The name 'Segments Center Error Right [pixel]'.
        """
        return 'Segments Center Error Right [pixel]'

    def compute(self, gt, pred, computed_metric_results, image_metadata):
        """
        Calculates the absolute distance between the ground truth and predicted centroid for all segments on the right side.
        Parameters:
